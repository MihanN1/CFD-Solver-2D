...
bool writeSectionAdapterOBJ(const std::filesystem::path& filename,
    ...
    error.clear();
    if (contour.size() < 3) {
        return fail(error, "section adapter requires at least three points");
    }
    ...
    if (std::abs(signedAreaTwice) <=
        std::numeric_limits<double>::epsilon() * span * span) {
    ...
    try {
        ...
        output << "# Derived from the selected GUI 2D contour; guarded repairs may apply.\n";
        ...
        for (std::size_t index = 0; index < pointCount; ++index) {
            ...
            const std::size_t bottomCurrent = index + 1;
            const std::size_t bottomNext = next + 1;
            const std::size_t topCurrent = pointCount + index + 1;
            const std::size_t topNext = pointCount + next + 1;
            output << "f "
                   << bottomCurrent << ' '
                   << bottomNext << ' '
                   << topNext << '\n';
