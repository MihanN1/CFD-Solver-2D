...
struct Slider {
    ...
    void draw(sf::RenderTarget& target,
        ...
        const std::string display =
            editing ? inputText + "|" : formatValue(value, integer, unit);
        if (editing) {
        ...
        sf::RectangleShape fill(
            {track.size.x * fraction, track.size.y});
...
struct ChildProcess {
    ...
    bool terminate(std::string* error = nullptr) {
        ...
        if (active && process != nullptr) {
            if (!GetExitCodeProcess(process, &exitCode)) {
                if (error != nullptr) {
                    *error = "Cannot inspect friend solver. Windows error " +
            ...
            if (exitCode == STILL_ACTIVE) {
                if (!TerminateProcess(process, ERROR_CANCELLED)) {
                    if (error != nullptr) {
                        *error = "Cannot stop friend solver. Windows error " +
                            std::to_string(GetLastError()) + ".";
                    }
                    return false;
                }
                if (WaitForSingleObject(process, 5000) != WAIT_OBJECT_0) {
                    if (error != nullptr) {
                        *error = "Friend solver did not stop within 5 seconds.";
                    }
                    return false;
                }
            }
    ...
    bool start(
        ...
        try {
            ...
            error = "Cannot encode friend solver arguments: " +
        ...
        CloseHandle(childError);
        if (!created) {
            error = "Cannot start friend solver. Windows error " +
                    std::to_string(windowsError) + ".";
            return false;
        }
        ...
        (void)arguments;
        (void)runDirectory;
        error = "External friend-solver launch is implemented for Windows only.";
        return false;
#endif
    }
...
std::filesystem::path chooseSolverExecutable(
    ...
    dialog.lpstrFile = filename.data();
    dialog.nMaxFile = static_cast<DWORD>(filename.size());
    dialog.lpstrTitle = L"Select friend solver executable";
...
std::filesystem::path runsRoot() {
    ...
    return root / "CFDMaskUIOptimized" / "runs";
}

std::filesystem::path createRunDirectory(std::string& error) {
    const auto timestamp =
        std::chrono::high_resolution_clock::now().time_since_epoch().count();
    const std::filesystem::path directory =
        runsRoot() / ("run-" + std::to_string(timestamp));
    std::error_code filesystemError;
...
public:
    Implementation(
        ...
        : executablePath_(std::move(executablePath)),
          ...
          friendSolverExecutable_(resolveFriendSolverExecutable(
              executablePath_,
              std::filesystem::path(CFD_SOLVER_EXE))),
          sliders_{
              Slider{"Wind speed", "m/s", 0.0, 50.0, 1.0, false, false},
              ...
              Slider{"Domain Lx", "m", 0.1, 10.0, 1.0, false, false},
              Slider{"Domain Ly", "m", 0.1, 10.0, 1.0, false, false},
              Slider{"Cells nx", "", 10.0, 500.0, 50.0, true, false},
              Slider{"Cells ny", "", 10.0, 500.0, 50.0, true, false},
              Slider{"Viscosity nu", "m2/s", 1e-6, 0.1, 0.01, false, true},
              Slider{"Density rho", "kg/m3", 0.1, 2000.0, 1.225, false, true},
              Slider{"CFL", "", 0.05, 1.0, 0.4, false, false},
              Slider{"Coarse SOR omega", "", 1.0, 1.95, 1.85, false, false},
              Slider{"Total time", "s", 0.0, 10.0, 0.0, false, false}
          } {
        std::string selectionError;
        const std::optional<std::filesystem::path> selected =
            readFriendSolverSelection(
                solverSelectionFile_,
                selectionError);
        if (selected) {
            friendSolverExecutable_ = *selected;
...
private:
    ...
    void updateLayout(sf::Vector2u size) {
        ...
        stopRenderButton_.bounds = {{400.0f, 10.0f}, {132.0f, 34.0f}};
        revealVtkButton_.bounds = {{540.0f, 10.0f}, {190.0f, 34.0f}};
        solverExeButton_.bounds = {{738.0f, 10.0f}, {196.0f, 34.0f}};
        importButton_.bounds = {{18.0f, 60.0f}, {294.0f, 34.0f}};
        generateButton_.bounds = {
            {18.0f, height - 62.0f},
            {294.0f, 38.0f}
        };

        const float firstSliderY = 126.0f;
        const float rowHeight =
            std::min(
                ...
                std::max(
                    39.0f,
                    (height - 230.0f) /
                        static_cast<float>(sliders_.size())));
        for (std::size_t index = 0; index < sliders_.size(); ++index) {
            sliders_[index].track = {
                {24.0f, firstSliderY + static_cast<float>(index) * rowHeight},
                {278.0f, 5.0f}
            };
        }
    ...
    void syncControlState() {
        ...
        generateButton_.enabled =
            !geometry_.empty() && !solverProcess_.active && !loadingResults;
        openVtkButton_.enabled = !solverProcess_.active && !loadingResults;
        stopRenderButton_.enabled = solverProcess_.active;
    ...
    void handleMousePressed(
        ...
        if (button == sf::Mouse::Button::Left &&
            ...
            selectFriendSolverExecutable();
            return;
        }
        if (button == sf::Mouse::Button::Left &&
            stopRenderButton_.hit(position)) {
            stopRenderAndLoadFrames();
            return;
        }
            ...
            if (result.has_value()) {
                if (*result == 0) {
                    ...
                    status_ = solverError.empty()
                                  ? "Friend solver failed with exit code " +
                                        std::to_string(*result) + "."
                                  : "Friend solver failed: " + solverError;
                }
    ...
    void refreshSolverProgress() {
        try {
            ...
            status_ =
                "Friend solver is running. Saved " +
                std::to_string(paths.size()) +
                " VTK frame(s). Closing the GUI cancels this run.";
        } catch (const std::exception& exception) {
            status_ =
                "Friend solver is running. Progress scan failed: " +
                std::string(exception.what());
        }
    }

    void stopRenderAndLoadFrames() {
        if (!solverProcess_.active) {
            status_ = "No friend solver render is active.";
            return;
        }
        std::string error;
        ...
        try {
            const std::vector<std::filesystem::path> paths =
                VtkFrameParser::discoverFrames(currentRunDirectory_);
            if (paths.empty()) {
                status_ = "Stopped friend solver; no VTK frames were saved.";
                return;
            }
            loadResultPaths(paths, ResultOrigin::StoppedFriendSolverRun);
        } catch (const std::exception& exception) {
            status_ = std::string("Stopped friend solver. VTK load failed: ") +
                exception.what();
        }
    }
    ...
    FriendRunConfig friendRunConfig() const {
        FriendRunConfig config;
    ...
    void openVtkFiles() {
        if (solverProcess_.active) {
            status_ =
                "Cannot import VTK frames while the friend solver is running.";
            return;
        }
        std::string error;
    ...
    void selectFriendSolverExecutable() {
        if (solverProcess_.active) {
            status_ =
                "Cannot change the friend solver while it is running.";
            return;
        }
        std::string error;
        const std::filesystem::path selected =
        ...
        if (!validateFriendSolverExecutable(executable, error)) {
            status_ = "Cannot select friend solver: " + error;
            return;
        }

        friendSolverExecutable_ = executable;
        if (!writeFriendSolverSelection(
                solverSelectionFile_,
                friendSolverExecutable_,
                error)) {
            status_ =
                "Selected friend solver for this session, but could not "
                "persist it: " + error;
            return;
        }
        status_ =
            "Selected friend solver: " +
            friendSolverExecutable_.string() +
            ". Rebuild or replace that EXE; the GUI will use the new file "
            "without rebuilding.";
    }
    ...
    void generateAndRun() {
        const FriendRunConfig requestedConfig = friendRunConfig();
        std::string error;
        if (!validateFriendSolverExecutable(
                friendSolverExecutable_,
                error)) {
            status_ = "Cannot run friend solver: " + error;
            return;
        }
        if (!validateFriendRunConfig(requestedConfig, error)) {
            status_ = "Cannot run friend solver: " + error;
            return;
        }

        const MaskResult mask =
            geometry_.generateMask(sectionParameters());
        if (!mask.success) {
        ...
        if (!writeSectionAdapterOBJ(adapterFile, mask.contour, error)) {
            status_ = "Cannot write section adapter: " + error;
            return;
        }

        FriendRunConfig solverConfig = requestedConfig;
        ...
        if (!buildFriendSolverArguments(
                ...
                solverArguments,
                error)) {
            status_ = "Cannot build friend solver arguments: " + error;
            return;
        }
        ...
        if (!writeFriendSolverArguments(
                ...
                solverArguments,
                error)) {
            status_ = "Cannot write friend solver arguments: " + error;
            return;
        }
        ...
        selectedFrame_ = 0;
        status_ =
            "Preview contains " + std::to_string(mask.solidCellCount) +
            " solid cells. Friend solver started. Closing the GUI cancels " +
            "this run." +
            (requestedConfig.totalTime > 0.0
    ...
    void loadExternalResultInputs(
        ...
        try {
            ...
            for (const std::filesystem::path& input : inputs) {
                ...
                if (discovered.empty()) {
                    throw VtkParseError(
                        "Directory contains no solution_<step>.vtk frames: " +
                        input.string());
                }
                paths.insert(
                    paths.end(),
    ...
    void loadResultFrames() {
        try {
            ...
            loadResultPaths(paths, ResultOrigin::FriendSolverRun);
        } catch (const std::exception& exception) {
    ...
    void loadResultPaths(
        ...
        ResultOrigin origin) {
        const bool stopped =
            origin == ResultOrigin::StoppedFriendSolverRun;
        ...
        resultCatalogFuture_ = std::async(
            ...
            [paths, stopped, origin]() {
                ...
                if (origin == ResultOrigin::FriendSolverRun &&
    ...
    void commitResultCatalog(
        ...
        if (origin == ResultOrigin::FriendSolverRun &&
            ...
            throw VtkParseError(
                "Friend solver completed without the required initial "
                "and positive-step VTK frames");
        }
        const bool hasPreview =
            fromFriendSolver && !previewSolid_.empty();
        ...
        if (hasPreview && !previewMatches) {
            ...
            resultsWarning_ +=
                "MASK MISMATCH: friend VTK differs from GUI preview.";
        }
        ...
        status_ = hasPreview && !previewMatches
            ? "Adapter verification failed: friend VTK mask differs "
            ...
            : "Indexed " + std::to_string(frames_.size()) +
                  ...
                  (hasPreview
                       ? " Friend mask matches the GUI preview."
                       : "");
        if (stopped) {
            status_ = "Stopped friend solver. " + status_;
        }
        if (rejectedCount != 0) {
    ...
    void drawResultCells() {
        ...
        window_->draw(makeText(
            font_,
            "Solver step " + std::to_string(frame.frameNumber),
            14,
            {
    ...
    void drawResultTooltip() {
        ...
        constexpr float tooltipHeight = 92.0f;
    ...
    void drawVelocityVectors(
        ...
        const double referenceMagnitude =
            velocityRange_.available
                ? std::max(0.0, velocityRange_.maximum)
                : 0.0;
    ...
    void drawTopTabs() {
        ...
        stopRenderButton_.draw(*window_, font_);
    ...
    std::filesystem::path friendSolverExecutable_;
    ...
    std::string status_ =
        "Import a model or drop solution_<step>.vtk frames. Double-click "
